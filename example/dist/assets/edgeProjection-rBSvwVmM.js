import { V as Vector3, k as OrthographicCamera, B as Box3, t as Mesh, a2 as WebGLRenderTarget, C as Color, u as ShaderMaterial, x as Vector4, ah as NoBlending, ai as GLSL3, aj as Loader, ak as MeshStandardMaterial, Y as LineBasicMaterial, al as FileLoader, e as SRGBColorSpace, G as Group, M as Matrix4, R as Ray, n as BufferGeometry, o as BufferAttribute, ae as LineSegments, U as UniformsUtils, a as UniformsLib, a9 as WebGLRenderer, a6 as Scene, a7 as DirectionalLight, a8 as AmbientLight, ad as MeshBVH, ag as SAH, i as PerspectiveCamera, af as g } from "./bvhcastEdges-BzgT52m-.js";
...
import { g as getAllMeshes, P as ProjectionGenerator } from "./ProjectionGenerator-M7W92PRG.js";
import "./PlanarIntersectionGenerator-BzRVJOaf.js";
...
class MeshVisibilityCuller {
  ...
  async cull(objects) {
    ...
    const { renderer: renderer2, pixelsPerMeter } = this;
    ...
    box.getSize(size);
    const maxTextureSize = Math.min(renderer2.capabilities.maxTextureSize, 2 ** 13);
    ...
    camera2.far = box.max.y - box.min.y + camera2.near;
    camera2.position.y = box.max.y + camera2.near;
    const color = renderer2.getClearColor(new Color());
    const alpha = renderer2.getClearAlpha();
    ...
    for (let x = 0; x < tilesX; x++) {
      for (let y = 0; y < tilesY; y++) {
        camera2.left = box.min.x + stepX * x;
        camera2.top = -(box.min.z + stepZ * y);
        camera2.right = camera2.left + stepX;
        camera2.bottom = camera2.top - stepZ;
        ...
        for (let i = 0; i < objects.length; i++) {
          ...
          renderer2.render(idMesh, camera2);
        }
        renderer2.setClearColor(color, alpha);
        renderer2.setRenderTarget(renderTarget);
        renderer2.autoClear = autoClear;
        const buffer = await renderer2.readRenderTargetPixelsAsync(target, 0, 0, target.width, target.height, readBuffer);
...
class LDrawPartsGeometryCache {
  ...
  async processIntoMesh(info) {
    ...
    const processInfoSubobjects = async (info2, subobject = null) => {
      ...
      group3.userData.category = info2.category;
      group3.userData.keywords = info2.keywords;
      group3.userData.author = info2.author;
      group3.userData.type = info2.type;
      group3.userData.fileName = info2.fileName;
      info2.group = group3;
      ...
      for (let i = 0, l = subobjectInfos.length; i < l; i++) {
        ...
        if (subobjectInfo.isGroup) {
          ...
          group3.add(subobjectGroup);
          continue;
        }
        if (subobjectInfo.group.children.length) {
          group3.add(subobjectInfo.group);
        }
      ...
      if (subobject) {
        loader.applyMaterialsToMesh(group3, subobject.colorCode, info2.materials);
        group3.userData.colorCode = subobject.colorCode;
      }
      return info2;
    };
    ...
    const group2 = info.group;
    if (info.faces.length > 0) {
      group2.add(createObject(this.loader, info.faces, 3, false, info.totalFaces));
    }
    if (info.lineSegments.length > 0) {
      group2.add(createObject(this.loader, info.lineSegments, 2));
    }
    if (info.conditionalSegments.length > 0) {
      group2.add(createObject(this.loader, info.conditionalSegments, 2, true));
    }
    return group2;
  }
  ...
  async getCachedModel(fileName) {
    if (fileName !== null && this.hasCachedModel(fileName)) {
      const key = fileName.toLowerCase();
      const group2 = await this._cache[key];
      return group2.clone();
    } else {
      return null;
    }
...
class LDrawLoader extends Loader {
  ...
  setConditionalLineMaterial(type) {
    ...
    this.missingConditionalEdgeColorMaterial = new this.ConditionalLineMaterial({ name: Loader.DEFAULT_MATERIAL_NAME, fog: true, color: 16711935 });
    return this;
  }
  /**
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    fileLoader.load(url, (text) => {
      ...
      this.partsCache.parseModel(text).then((group2) => {
        this.applyMaterialsToMesh(group2, MAIN_COLOUR_CODE, this.materialLibrary, true);
        this.computeBuildingSteps(group2);
        group2.userData.fileName = url;
        onLoad(group2);
      }).catch(onError);
    }, onProgress, onError);
  }
  ...
  applyMaterialsToMesh(group2, parentColorCode, materialHierarchy, finalMaterialPass = false) {
    const loader = this;
  ...
  computeBuildingSteps(model2) {
    ...
    model2.userData.numBuildingSteps = stepNumber + 1;
  }
}
...
async function init() {
  ...
  if (window.location.hash === "#lego") {
    ...
    model = await loader.setPartsLibraryPath("https://raw.githubusercontent.com/gkjohnson/ldraw-parts-library/master/complete/ldraw/").loadAsync("https://raw.githubusercontent.com/mrdoob/three.js/dev/examples/models/ldraw/officialLibrary/models/1621-1-LunarMPVVehicle.mpd_Packed.mpd");
  ...
  model.traverse((c) => {
    if (c.geometry && !c.geometry.boundsTree) {
      ...
      c.geometry.groups.forEach((group2) => {
        if (group2.count === Infinity) {
          group2.count = elCount - group2.start;
        }
      });
  ...
  group.add(model);
  group.updateMatrixWorld(true);
  projection = new LineSegments(new BufferGeometry(), new LineBasicMaterial({ color: 197379, depthWrite: false }));
  drawThroughProjection = new LineSegments(new BufferGeometry(), new LineBasicMaterial({ color: 13290186, depthWrite: false }));
...
//# sourceMappingURL=edgeProjection-rBSvwVmM.js.map
